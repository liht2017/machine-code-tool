...
- **CPU 序列号重复概率较大**：本工具在 Windows 上使用 WMI 的 `ProcessorId`（由 CPUID 推导，同型号 CPU 往往相同）；在 macOS 上使用 CPU 型号字符串的哈希，**同型号 CPU 会得到完全相同的值**。因此多台同批次或同型号机器出现「CPU 序列号」相同属于正常现象。
- **设备唯一性以「机器码」为准**：接口返回的**机器码**由 **MAC 地址 + CPU + 硬盘序列号 + 主板序列号**（Windows 下还包含系统 UUID、BIOS 序列号等）共同参与哈希生成。即使 CPU 项重复，只要 MAC、硬盘、主板等有一项不同，**机器码仍会不同**，可区分设备。
- **业务侧建议**：以 `GET /api/machine-code` 返回的**机器码**作为设备唯一标识进行校验或绑机，不要仅依据界面展示的「CPU 序列号」判重。
